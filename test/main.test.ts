...
import { generateRandomGrid, applyRules, getLiveNeighborCount } from '../src/utils';
import type { CellCharState, Dimensions } from '../src/types';
...
describe('applyRules()', () => {
  test('the new grid should have all cells alive', () => {
    const dimensions: Dimensions = [4, 4];
    // set every cell to be alive
    const cellCharState: CellCharState = ['X', 'X'];
...
describe('getLiveNeighborCount()', () => {
  test('center cell should return all neighbors alive', () => {
    const dimensions: Dimensions = [5, 5];
    const cellCharState: CellCharState = ['X', 'X'];
    const grid = generateRandomGrid(dimensions, cellCharState);
